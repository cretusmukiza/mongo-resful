...
public class PersonDALImpl implements PersonDAL {
    ...
    @Override
    public List<Person> getAllPersonsPaginated(int pageNumber, int pageSize) {
    ...
    @Override
    public List<Person> findByFavouriteBook(String favouriteBook){
        Query query = new Query();
        query.addCriteria(Criteria.where("favouriteBooks").in(favouriteBook));
        return mongoTemplate.find(query,Person.class);
    }
